...
function getProdDatasetPath() {
    ...
    if (typeof prodDSPath !== 'string') throw new Error('Production dataset path is not set in the environment variable!')
...
export function sample<T>(transformFeature: Transformer<T>) {
    ...
    if (requestedUniqueId) {
        ...
        if (!matchedSample) throw new Error("Cannot find sample with unique id: " + requestedUniqueId)
        return [matchedSample, samples.filter(e => e.fileName !== requestedUniqueId)] as const;    
    } else {
